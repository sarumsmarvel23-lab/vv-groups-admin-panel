...
import { getStayById, updateStay } from "../../api/stay";
...
const UpdateStay = () => {
  ...
  const [formData, setFormData] = useState({
    name: "", description: "", location: "", type: "Homestay",
    pricePerNight: 0, rating: 0, reviews: 0, amenities: "",
    status: "Active", youtubeLink: ""
  });
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (res.success) {
        const d = res.result;
        // Convert amenities array to comma string
        if (Array.isArray(d.amenities)) d.amenities = d.amenities.join(", ");
        // Load rooms into state (each room is already an object)
        if (Array.isArray(d.rooms) && d.rooms.length > 0) {
          setRooms(d.rooms.map(r => ({
            name: r.name || "",
            sleeps: r.sleeps !== undefined ? String(r.sleeps) : "",
            price: r.price !== undefined ? String(r.price) : "",
          })));
        }
        // Remove the rooms key from formData since we manage it separately
        const { rooms: _rooms, ...rest } = d;
        setFormData(rest);
      }
    };
    fetch();
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      toast({ type: "success", message: "Stay updated successfully!" }, dispatch);
      navigate("/stays");
    } else {
      toast({ type: "error", message: res.message || "Error updating stay" }, dispatch);
    }
  };

  return (
    <CCard>
      <CCardHeader><strong>Update Stay</strong></CCardHeader>
      <CCardBody>
        <CForm onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            <CFormInput id="pricePerNight" type="number" label="Price Per Night (₹)" value={formData.pricePerNight || 0} onChange={handleChange} />
          </div>
          <div className="mb-3">
            <CFormInput id="rating" type="number" step="0.1" min="0" max="10" label="Rating (e.g. 9.2)" value={formData.rating || 0} onChange={handleChange} />
          </div>
          <div className="mb-3">
            <CFormInput id="reviews" type="number" label="Number of Reviews" value={formData.reviews || 0} onChange={handleChange} />
          </div>
          <div className="mb-3">
