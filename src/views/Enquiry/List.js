...
import { getEnquiries, deleteEnquiry } from "../../api/enquiry";
...
const EnquiryList = () => {
  ...
  const fetchEnquiries = async ({
    ...
    try {
      ...
      const { success, result } = await getEnquiries(reqData);
      if (success) {
        setData(result.data);
    ...
    },
    {
      ...
      Cell: ({ value }) => (
        <div className="d-flex gap-2">
          <Link to={`/update-enquiry/${value}`} className="btn btn-sm btn-info text-white">View/Update</Link>
