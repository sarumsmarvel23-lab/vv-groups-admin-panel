...
//import api
import { SubloginHistoryApi } from "../../api/common";
import { orderXLS, orderCSV, suborderPDF } from "./downloader";
//import lib
...
const UserList = () => {
  ...
  const handleDoc = async type => {
    ...
    let { result, data } = await SubloginHistoryApi(docType);
    console.log("data--->",data)
    
    if (type && type === "pdf") {
      suborderPDF(result.pdfData);
  ...
  const fetchListData = async ({ pageIndex, pageSize, filters, sortBy }) => {
    try {
      ...
      const { success, result } = await SubloginHistoryApi(reqData);
      if (success) {
        setData(result.data);
